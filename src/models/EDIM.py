...
from neural_networks.gan import Discriminator
from utils.custom_typing import (
    EDIMOutputs,
    DiscriminatorOutputs,
    EDIMClassifierOutputs,
)
from neural_networks.classifier import Classifier, BiggerClassifier

from utils.custom_typing import EDIMSmallNORBOutputs
...
class EDIM(nn.Module):
    ...
    def __init__(
        ...
        self.left_cat_classifier = Classifier(feature_dim=exclusive_dim, output_dim=len(ATTRIBUTE_TYPES['cat']))
        self.right_cat_classifier = Classifier(feature_dim=exclusive_dim, output_dim=len(ATTRIBUTE_TYPES['cat']))

        self.elevation_classifier = Classifier(feature_dim=exclusive_dim, output_dim=len(ATTRIBUTE_TYPES['elevation']))
        self.lightning_classifier = Classifier(feature_dim=exclusive_dim, output_dim=len(ATTRIBUTE_TYPES['lightning']))

    def forward_generator(self, x: torch.Tensor, y: torch.Tensor) -> EDIMSmallNORBOutputs:
        ...
        Returns:
            EDIMSmallNORBOutputs: Generator outputs
        """
    ...
    def forward_classifier(self, edim_outputs: EDIMSmallNORBOutputs) -> EDIMClassifierOutputs:
        """Forward pass of the classifiers

        Args:
            edim_outputs (EDIMSmallNORBOutputs): Outputs from the generator
        ...
        left_cat_logits = self.left_cat_classifier(out.exclusive_x.detach())
        right_cat_logits = self.right_cat_classifier(out.exclusive_y.detach())
        elevation_logits = self.elevation_classifier(out.exclusive_x.detach())
        lightning_logits = self.lightning_classifier(out.exclusive_y.detach())

        return EDIMClassifierOutputs(
            left_cat_logits=left_cat_logits,
            right_cat_logits=right_cat_logits,
